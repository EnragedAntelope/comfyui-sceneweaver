...
SceneWeaver is a scene engine built to span genres. **Sci-fi and fantasy ship
now**; horror is next, on the same coherence engine
([roadmap](https://github.com/EnragedAntelope/comfyui-sceneweaver/blob/main/docs/genre-roadmap.md)).
And the genres mix: wire a fantasy gorgon into a sci-fi station, or a sci-fi mech
into a dragon's hoard.
...
does not move, a ruined keep shows no lit windows, a small creature does no
giant's deed, nothing burns under the sea, and a sea serpent stays in the water.
...
values go at the top level; fantasy values go in a `"fantasy"` section. To see what
a pool already holds:
...
genre module, which is how fantasy arrived without an engine change. **Horror is
next**, with a gore filter that works like Peaceful / Conflict, followed by a tone
control for fantasy. The plan is in
[`docs/genre-roadmap.md`](https://github.com/EnragedAntelope/comfyui-sceneweaver/blob/main/docs/genre-roadmap.md).

**Which genre do you want after horror?**
